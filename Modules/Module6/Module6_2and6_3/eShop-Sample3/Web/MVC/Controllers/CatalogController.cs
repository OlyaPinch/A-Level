...
public class CatalogController : Controller
{
    ...
    public async Task<IActionResult> Index(int brandFilterApplied, int typesFilterApplied, int? page, int? itemsPage)
    {
        ...
        var catalog =
            await _catalogService.GetCatalogItems(page.Value, itemsPage.Value, brandFilterApplied, typesFilterApplied);
        if (catalog == null)
        {
            return View("Error");
        }
        ...
        var vm = new IndexViewModel()
        {
            ...
            Brands = await _catalogService.GetBrands(0, int.MaxValue),
